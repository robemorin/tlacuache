    ...
    print(v) {
        ...
        for (let k = 1; k < n; ++k) {
            if (!Array.isArray(v[k])) {
                ...
                if (v[k][0] != 0) {
                    if (k == n - 1) {
                        S += ` ${(v[k][0] * v[k][1] < 0 ? "-" : "+")} \\frac{${Math.abs(v[k][0])}}{${Math.abs(v[k][1])}}`
                    } else if (k == n - 2) {
                        S += `${(v[k][0] * v[k][1] < 0 ? "-" : "+")} \\frac{${Math.abs(v[k][0])}}{${Math.abs(v[k][1])}} x`
...
export const stat = {
    ...
    normalcdf(lower, upper, mu, sigma) {
        ...
        const z1 = (lower - mu) / (sigma * Math.sqrt(2));
        const z2 = (upper - mu) / (sigma * Math.sqrt(2));
    ...
    invNorm(area, mu, sigma) {
        ...
        let p = area;
        if (p < 0.5) {
            return mu - sigma * rational_approximation(Math.sqrt(-2.0 * Math.log(p)));
        } else {
            return mu + sigma * rational_approximation(Math.sqrt(-2.0 * Math.log(1.0 - p)));
        }
    },
    chiCuadrada(observed, expected) {
        let chi2 = 0;
...
const tlacu = (function () {
    ...
    return {
        ...
        mediatriz: function (p1, p2) {
            ...
            return {
                general: `${A == 1 ? '' : A} x ${B < 0 ? '-' : '+'} ${Math.abs(B) == 1 ? '' : Math.abs(B)} y ${C < 0 ? '-' : '+'} ${Math.abs(C)} = 0`,
