...
// tlacuache-elements.js
import * as tlacu from './tlacuache-modulo.mjs'

export class tlacuache_cuartil extends HTMLElement {
    constructor() {
      super();
      // element created
      this.Q 
...
customElements.define('tlacuache-cuartil',tlacuache_cuartil)
...
customElements.define('tlacuache-dist-normal', tlacuache_distNormal)
...
customElements.define('tlacuache-milimetrado', tlacuache_Milimetrado)
...
class tlacuache_renglon extends HTMLElement
  ...
  stroke = ".7" stroke2 = ".2"/&gt;
      ...
      let tabla = `<table class="tlacuache-renglon" style="border-collapse:collapse; border-spacing:0px; width:100%; height:${this.alto*this.n}px; border:1px solid ${this.color};">`
...
customElements.define('tlacuache-renglon',tlacuache_renglon)
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      switch(name){
        ...
        case 'color':
            ...
            break
        case 'ddx':
          this.ddx = tlacu.graph.tick(this.xlim[0],this.xlim[1],eval(newValue))
          break
        case 'ddy':
          this.ddy = tlacu.graph.tick(this.ylim[0],this.ylim[1],eval(newValue))
          break
        case 'dx':
            newValue = tlacu.graph.tick(this.xlim[0],this.xlim[1],eval(newValue))
        case 'xtick'://Falta probar
            this.xtick = eval(`[${newValue}]`)
            break
        case 'dy':
          newValue = tlacu.graph.tick(this.ylim[0],this.ylim[1],eval(newValue))
...
customElements.define('tlacuache-ejes',tlacuache_ejes)
...
customElements.define('tlacuache-histograma',tlacuache_histograma)
...
customElements.define('tlacuache-plot',tlacuache_plot)
...
customElements.define('tlacuache-venn',tlacuache_venn)
...
customElements.define('tlacuache-poli_fa',tlacuache_poligonoFA)
export class tlacuache_tabla extends HTMLElement
  {
    constructor() {
      super();
      // element created
...
function processMath(element) {
  if (window.MathJax) {
    ...
    console.warn('MathJax no está cargado aún');
  }
}
